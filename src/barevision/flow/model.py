...
class HierarchicalEmbeddingModel(nnx.Module):
    ...
    def __init__(
        ...
        *,
        ...
        # Build pyramid levels as separate attributes for nnx compatibility
        for i in range(num_levels):
            ...
            # Store as level_0_spatial, level_0_proj, level_1_spatial, etc.
            setattr(self, f"level_{i}_spatial", spatial_conv)
            setattr(self, f"level_{i}_proj", pointwise_conv)
    ...
    def __call__(self, x: jnp.ndarray) -> List[jnp.ndarray]:
        ...
        for i in range(self.num_levels):
            # Get convolutions for this level
            spatial_conv = getattr(self, f"level_{i}_spatial")
            pointwise_conv = getattr(self, f"level_{i}_proj")
