...
class TestLocalContrastNormalization:
    """Tests for LCN enable/disable functionality."""

    def test_lcn_enabled_creates_mean_conv(self):
        """Test that mean_conv exists when LCN is enabled."""
        model = StemBlock(
            hidden_dim=32,
            embed_dim=16,
            num_groups=8,
        ...
        # mean_conv should exist
        assert hasattr(model, "mean_conv")
        assert model.mean_conv is not None

    def test_lcn_disabled_no_mean_conv(self):
        """Test that mean_conv doesn't exist when LCN is disabled."""
        model = StemBlock(
            hidden_dim=32,
            embed_dim=16,
            num_groups=8,
        ...
        # mean_conv should not exist
        assert not hasattr(model, "mean_conv")

    def test_lcn_disabled_produces_different_embeddings(self):
        """Test that disabling LCN produces different embeddings."""
        model_lcn = StemBlock(
            hidden_dim=32,
            embed_dim=16,
            num_groups=8,
        ...
        emb_lcn, _ = model_lcn(x)
        emb_no_lcn, _ = model_no_lcn(x)

        # Embeddings should be different
        assert not jnp.allclose(emb_lcn, emb_no_lcn, atol=1e-5)

    def test_lcn_disabled_preserves_dimensions(self):
        """Test that disabling LCN still produces correct dimensions."""
        model = StemBlock(
            hidden_dim=32,
            embed_dim=16,
            num_groups=8,
        ...
        assert embedding.shape == (1, 79, 79, 16)
        assert downsampled.shape == (1, 39, 39, 32)

    def test_lcn_setting_propagates_to_hierarchical_model(self):
        """Test that LCN setting propagates from settings to model."""
        # With LCN enabled
        model_lcn = HierarchicalEmbeddingModel(
        ...
        # With LCN disabled
        model_no_lcn = HierarchicalEmbeddingModel(
            EmbeddingModelSettings(
                hidden_dim=32,
                embed_dim=16,
                num_groups=8,
        ...
        # Check all blocks have correct configuration
        for block in model_lcn.blocks:
            assert block.use_local_contrast_normalization == True
            assert hasattr(block, "mean_conv")

        for block in model_no_lcn.blocks:
            assert block.use_local_contrast_normalization == False
            assert not hasattr(block, "mean_conv")

    def test_lcn_disabled_parameter_count(self):
        """Test parameter count without LCN."""
        ...
        # Without LCN, we save the mean_conv parameters:
        # Each mean_conv: 32*9 + 32 = 320 params
        # 3 levels = 960 params saved
        # Original with LCN: 7248
        # Expected without LCN: 7248 - 960 = 6288
        assert param_count == 6288, f"Expected 6288 parameters, got {param_count}"

    def test_lcn_comparison_same_input_output_shapes(self):
        """Test that both LCN modes produce same output shapes."""
        model_lcn = HierarchicalEmbeddingModel(
            EmbeddingModelSettings(
                hidden_dim=32,
                embed_dim=16,
        ...
        pyramid_lcn = model_lcn(x)
        pyramid_no_lcn = model_no_lcn(x)

        # All levels should have same shapes
        for i, (emb_lcn, emb_no_lcn) in enumerate(zip(pyramid_lcn, pyramid_no_lcn)):
            assert emb_lcn.shape == emb_no_lcn.shape, f"Level {i} shapes differ"
