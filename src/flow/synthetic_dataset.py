...
class SyntheticFlowDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        num_blobs = np.random.randint(
            self.num_blobs_range[0], self.num_blobs_range[1] + 1
        )
        blobs = []
            ...
            blobs.append(
                {
                    ...
                    "size": np.random.randint(
                        self.blob_size_range[0], self.blob_size_range[1] + 1
                    ),
                    "scale": np.random.uniform(
                        self.noise_scale_range[0], self.noise_scale_range[1]
                    ),
                    "pos1": (
        ...
        for blob in blobs:
            ...
            x2, y2 = int(np.clip(x1 + dx, 0, self.img_size - size)), int(
                np.clip(y1 + dy, 0, self.img_size - size)
            )
...
if __name__ == "__main__":
    ...
    try:
        dataset = SyntheticFlowDataset(
            img_size=18, bg_noise_scale=16.0, frame_noise_std=0.05  # <-- MODIFIED
        )
