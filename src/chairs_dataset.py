...
class ChairsV0Dataset(Dataset):
    def __init__(self, dataset_root, split='train', img_size=32, patch_size=4):
        ...
        self.patch_size = patch_size  # e.g., 4
        self.grid_size = img_size // patch_size  # e.g., 8

        self.sample_names = [
            f.split('.')[0] for f in os.listdir(self.t0_dir) if f.endswith('.png')
        ]

        # Images are [0, 1] (no normalization)
    ...
    def __getitem__(self, idx):
        ...
        flow_orig, _ = readPFM(flow_path)

        # --- 1. Center Crop (as before) ---
        crop_size = 384
        ...
        # --- 2. Process Images (as before) ---
        img_t0 = self.img_transform(img_t0_cropped)  # (3, 32, 32)
        img_t1 = self.img_transform(img_t1_cropped)  # (3, 32, 32)

        # --- 3. Process Flow (YOUR NEW LOGIC) ---

        # a. Rescale flow vectors (as before)
        # Scale factor = (target_size / original_size)
        scale_factor = self.img_size / crop_size  # 32 / 384
        flow_scaled = flow_cropped * scale_factor

        # b. Convert to (B, C, H, W) tensor
        # (H, W, C) -> (C, H, W) -> (B, C, H, W)
        flow_tensor = torch.from_numpy(flow_scaled).permute(2, 0, 1).unsqueeze(0)
        # Shape is (1, 2, 384, 384)

        # c. --- THIS IS THE NEW STEP ---
        # Resize the *flow map* from 384x384 down to 32x32
        flow_resized = F.interpolate(
            flow_tensor,
            size=(self.img_size, self.img_size),  # (32, 32)
            mode='bilinear',
            align_corners=False
        )
        # Shape is now (1, 2, 32, 32)

        # d. --- THIS IS THE NEW POOLING ---
        # Now, average-pool the 32x32 map down to our 8x8 grid
        # The kernel size is now self.patch_size (e.g., 4)
        ...
        # e. Reshape for JAX (as before)
        P = self.grid_size * self.grid_size  # 64
        # (2, 8, 8) -> (2, 64) -> (64, 2)
        flow_target_jax = flow_target_torch.reshape(2, P).T

        return img_t0, img_t1, flow_target_jax
